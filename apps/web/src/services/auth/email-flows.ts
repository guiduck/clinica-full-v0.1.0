...
import { AUTH_TOKEN_KINDS, consumeAuthToken, issueAuthToken } from "./auth-tokens";
...
export async function requestPasswordReset(email: string) {
  ...
  if (!user) return;
  const token = await issueAuthToken(user.id, AUTH_TOKEN_KINDS.passwordReset, 30);
  const url = `${getPublicAppUrl()}/redefinir-senha?token=${encodeURIComponent(token)}`;
  await sendTransactionalEmail({ to: user.email, ...passwordResetEmail(user.name, url) });
}
