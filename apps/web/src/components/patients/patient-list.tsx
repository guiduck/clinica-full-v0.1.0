...
const initials = (name: string) =>
  name
    ...
    .join("") || "?";
const unavailable = {
  key: "patients.mutation",
  mode: "unavailable",
  title: "Ação ainda não disponível",
  message:
    "A interface está pronta, mas esta alteração será conectada quando o service correspondente estiver disponível. Nenhum dado foi modificado.",
  affectedAction: "mutate",
} as const;

function patientStatusLabel(status: PatientSummary["status"]) {
  ...
  return (
    <main className="app-page space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Gestão da sua carteira clínica
          </p>
        </div>
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {patients.length
                  ...
                  : "Comece cadastrando o primeiro paciente da sua carteira clínica."}
              </p>
              ...
              <tbody>
                {filtered.map((patient) => (
                  <tr key={patient.id} className="border-b last:border-0">
                    ...
                    <td>
                      <p>{patient.email ?? "E-mail não informado"}</p>
                    ...
                    <td>
                      <Badge
                        tone={
                          patient.status === "ativo" ? "success" : "neutral"
                        }
                      >
                    ...
                    <td>
                      <div className="flex justify-end gap-1">
                        <CapabilityNotice
                          descriptor={{
                            ...unavailable,
                            key: "patients.whatsapp",
                            title: "WhatsApp ainda não conectado",
                            message:
                              "O envio manual será habilitado quando o service de mensagens estiver disponível. Nenhuma mensagem foi enviada.",
                            affectedAction: "send",
                          }}
                          trigger={
                            <span>
                              <Action label="WhatsApp" icon={MessageCircle} />
                            </span>
                          }
                        />
                        <Action
                          ...
                          onClick={() =>
                            router.push(
                              `/pacientes/${patient.id}?tab=financeiro`,
                            )
                          }
                        />
                        <Action
                          ...
                          onClick={() =>
                            router.push(
                              `/pacientes/${patient.id}/financeiro?focus=payment`,
                            )
                          }
                        />
                        <CapabilityNotice
                          ...
                          trigger={
                            <span>
                              <Action label="Editar" icon={Edit3} />
                            </span>
                          }
                        />
                        <CapabilityNotice
      ...
      <PatientWizard
        ...
        onOpenChange={(value) => {
          setWizard(value);
          if (!value && params.get("new")) router.replace("/pacientes");
        }}
      />
    </main>
  );
