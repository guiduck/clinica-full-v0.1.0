  ...
  const nodes: Record<DashboardSectionKey, React.ReactNode> = {
    ...
    finance: (
      <Card className="p-5">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2"><div className="inline-flex rounded-md border p-0.5 text-xs">{PERIOD_OPTIONS.map((option) => <button type="button" key={option.value} onClick={() => setPeriod(option.value)} className={cn("min-h-11 rounded px-2 py-1", period === option.value ? "bg-primary text-primary-foreground" : "text-muted-foreground")}>{option.label}</button>)}</div><Button variant="ghost" size="icon" onClick={toggleFinancial} aria-label={hidden ? "Exibir valores" : "Ocultar valores"}>{hidden ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</Button></div>
        </div>
    ...
    messages: (
      <Card className="p-5">
        ...
        {messageVisible ? <div className="flex items-center justify-between gap-3 border-t py-3"><div><p className="text-sm font-medium">Boas-vindas — paciente</p><p className="text-xs text-muted-foreground">WHATSAPP • aguardando confirmação</p></div><div className="flex gap-1"><Button size="sm" variant="outline" onClick={() => toast.info("O envio será conectado quando o serviço de mensagens estiver disponível.")}>Revisar e enviar</Button><Button size="icon" variant="ghost" className="text-destructive" aria-label="Remover mensagem da fila" onClick={() => setMessageVisible(false)}><Trash2 className="size-4" /></Button></div></div> : <p className="py-4 text-sm text-muted-foreground">Nenhuma mensagem programada no momento.</p>}
      </Card>
    ),
    patients: (
