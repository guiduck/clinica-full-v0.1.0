...
type Period = DashboardFinancePeriod;
...
export function DashboardView({
  ...
  const onDrop = (result: DropResult) => {
    ...
    React.startTransition(() => {
      void updateUserUiPreferenceAction({
        operation: "set_dashboard_order",
        sectionKeys: next,
      });
    });
  };
  const toggleFinancial = () => {
    ...
    React.startTransition(() => {
      void updateUserUiPreferenceAction({
        operation: "set_financial_visibility",
        hidden: next,
      });
    });
  };
  ...
  const chartPoint = (key?: string) => {
    ...
    router.push(dashboardFinanceMonthHref(year, month));
  };
  ...
  const nodes: Record<DashboardSectionKey, React.ReactNode> = {
    appointments: (
      <section className="grid gap-6 lg:grid-cols-[2fr_1fr]">
            ...
            <div className="space-y-4">
              <div className="flex gap-2 text-xs">
                <span className="rounded-full bg-primary/15 px-2 py-1 font-medium text-primary">
                  <span className="status-dot mr-1 bg-primary" />
                  {dashboard.todayCount} hoje
                </span>
                <span className="rounded-full bg-info/15 px-2 py-1 font-medium text-info">
                  {dashboard.tomorrowCount} amanhã
                </span>
              </div>
              <ul className="divide-y">
                {upcoming.map((item, index) => (
                  <li
                    ...
                    className={cn(
                      "py-3",
                      index === 0 && "border-l-2 border-primary pl-2",
                    )}
                  >
                    <Link
                    ...
                    >
                      <Avatar className="size-9">
                        <AvatarFallback className="bg-brand-soft text-xs text-primary">
        ...
        <Card className="min-h-80 p-6">
          ...
          <p className="text-sm text-muted-foreground">
            Clique para ir direto à seção
          </p>
          <p className="py-8 text-sm text-muted-foreground">
            Nenhum lembrete no momento. Tudo em dia!
          </p>
        </Card>
      </section>
    ),
      ...
      <Card className="p-5">
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Resumo financeiro deste mês e tendência de saldo
            </p>
          </div>
        ...
        {hidden ? (
          <div className="grid h-56 place-items-center text-sm text-muted-foreground">
            Valores ocultos. Clique no ícone para exibir.
          </div>
        ) : (
          <div className="grid gap-6 lg:grid-cols-2">
                  ...
                  <PieChart>
                    <Pie
                      data={[
                        {
                          name: "Receitas",
                          value: financeSummary.effectiveRevenue,
                        },
                        {
                          name: "Despesas",
                          value: financeSummary.effectiveExpense,
                        },
              ...
              <div className="grid grid-cols-2 text-center text-sm">
                <div>
                  ...
                  <strong className="text-success">
                    {formatBRL(financeSummary.effectiveRevenue / 100)}
                  </strong>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Despesa</p>
                  <strong className="text-warning">
                    {formatBRL(financeSummary.effectiveExpense / 100)}
                  </strong>
                </div>
              </div>
            </div>
            <div>
              ...
              <p className="mb-1 text-[11px] text-muted-foreground">
                Clique em um ponto para filtrar no financeiro
              </p>
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
                      ...
                      activeDot={
                        ((props: {
                          ...
                          <circle
                            ...
                            onClick={() => chartPoint(props.payload?.key)}
                          />
                        )) as never
                      }
                    />
    ...
    patients: (
      <section
        id="tour-dashboard-summary"
        className="grid grid-cols-2 gap-4 lg:grid-cols-4"
      >
        <span className="sr-only">
          Resumo de {dashboard.activePatientCount} pacientes ativos
        </span>
        <SummaryCard
        ...
        />
        <SummaryCard
          href="/financeiro?period=1m"
          label="Saldo atual"
          value={
            hidden
              ? "••••••"
              : formatBRL(financeSummary.effectiveBalance / 100)
          }
          icon={Wallet}
          accent="bg-brand-soft text-primary"
        />
        <SummaryCard
          href="/financeiro?tab=receitas&period=1m"
          label="Receita prevista"
          value={
            hidden
              ? "••••••"
              : formatBRL(financeSummary.expectedRevenue / 100)
          }
          icon={TrendingUp}
          accent="bg-success/15 text-success"
        />
        <SummaryCard
          href="/financeiro?tab=receitas&status=pendentes"
          label="Receitas a confirmar"
          value={
            hidden
              ? "••••••"
              : formatBRL(financeSummary.expectedRevenue / 100)
  ...
  return (
    <div className="app-page space-y-6">
      ...
      {newsVisible ? (
        <Card className="relative overflow-hidden border-primary/25 bg-gradient-to-r from-brand-soft/80 to-card">
          <div className="flex min-h-36 items-stretch">
            <div className="hidden w-36 place-items-center bg-primary/5 sm:grid">
              <Sparkles className="size-12 text-primary/35" />
            </div>
            <div className="flex-1 p-5">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Agora você filtra receitas, despesas e gráficos por período
                direto pela URL. Resumo do mês selecionado, layout
                personalizável e cobranças no cadastro de paciente.
              </p>
              <Link
                href="/financeiro"
                className="mt-2 inline-flex items-center gap-1 text-sm font-medium text-primary"
              >
                Conhecer a feature <ChevronRight className="size-4" />
              </Link>
            </div>
            <Button
              variant="ghost"
                ...
                >
                  {(item, snapshot) => (
                    <div
                      ...
                      className={cn(
                        "relative",
                        snapshot.isDragging && "opacity-90 shadow-elevated",
                      )}
                    >
                      {editing ? (
                        <div
                          {...item.dragHandleProps}
                          className="mb-2 flex cursor-grab items-center gap-2 rounded-md border border-dashed bg-card px-3 py-2 text-xs font-medium text-muted-foreground"
                        >
...
function SummaryCard({
  ...
  return (
    <Link href={href}>
      <Card className="h-full p-5 transition-[border-color,box-shadow] hover:border-primary/40 hover:shadow-sm">
        <div className="flex items-start gap-3">
          <p className="min-w-0 flex-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            {label}
          </p>
          <span
            className={cn("grid size-9 place-items-center rounded-lg", accent)}
          >
