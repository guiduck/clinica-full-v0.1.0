...
const unavailable = {
  key: "finance-ledger-write",
  mode: "unavailable" as const,
  affectedAction: "mutate" as const,
  title: "Serviço financeiro em preparação",
  message:
    "A experiência já está reconstruída, mas este lançamento não será salvo enquanto o livro-caixa financeiro não estiver conectado. Nenhum dado fictício foi criado.",
};
...
function isoFromBR(value: string) {
  const match = /^(\d{2})\/(\d{2})\/(\d{4})$/.exec(value);
  return match ? `${match[3]}-${match[2]}-${match[1]}` : "";
}
function brFromIso(value: string) {
...
function rangeFor(period: string, from: string, to: string) {
  ...
  if (period === "custom")
    return {
      start: from
        ? new Date(`${isoFromBR(from)}T00:00:00`)
        : new Date(1970, 0, 1),
      end: to ? new Date(`${isoFromBR(to)}T23:59:59`) : new Date(2999, 11, 31),
    };
  const end =
    period === "week"
...
export function FinanceDashboard({
  ...
  const tab = (
    tabs.some((item) => item.key === search.get("tab"))
      ? search.get("tab")
      : "todos"
  ) as TabKey;
  ...
  let selectedType: "receita" | "despesa" | undefined;
  if (tab === "receitas") selectedType = "receita";
  if (tab === "despesas") selectedType = "despesa";
  const filtered = filterFinanceEntries(entries, {
    type: selectedType,
    status:
      status === "todos"
        ? undefined
        : (status as FinanceEntryView["status"] | "pendentes"),
    category,
    query,
    start: range.start,
    end: range.end,
  ...
  const effectiveRevenue = summary.effectiveRevenue;
  const effectiveExpense = summary.effectiveExpense;
  const expected = summary.expectedRevenue;
  const chartData = [
    { name: "Receitas", value: effectiveRevenue },
    { name: "Despesas", value: effectiveExpense },
  ];
  const daily = Array.from({ length: 6 }, (_, index) => {
    const d = new Date();
    d.setMonth(d.getMonth() - (5 - index));
    const same = active.filter((item) => {
    ...
    const receitas = same
      .filter((x) => x.type === "receita" && x.status === "efetivado")
      .reduce((s, x) => s + x.valueCents / 100, 0);
    const despesas = same
      .filter((x) => x.type === "despesa" && x.status === "efetivado")
      .reduce((s, x) => s + x.valueCents / 100, 0);
    return {
  ...
  if (tab === "recibos") {
    primaryContent = (
      <EmptyPanel
        ...
        text="Selecione uma receita efetivada para emitir o primeiro recibo."
  ...
  if (isTable) {
    primaryContent = (
      <>
        <div className="mt-5 grid gap-3 md:grid-cols-[140px_150px_150px_160px_1fr]">
          <div>
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="1y">Ano</SelectItem>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                patchQuery({
                  from: isoFromBR(maskDate(e.target.value)) || null,
                  period: "custom",
                })
              }
            />
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                patchQuery({
                  to: isoFromBR(maskDate(e.target.value)) || null,
                  period: "custom",
                })
              }
            />
          ...
          <div>
            ...
            <Select
              value={status}
              onValueChange={(value) =>
                patchQuery({ status: value === "todos" ? null : value })
              }
            >
              <SelectTrigger className="mt-1">
                <SelectValue />
        ...
        <FinanceTable entries={filtered} onEdit={setEditingEntry} />
      </>
    );
  }
  const hasEffectiveEntries = Boolean(effectiveRevenue || effectiveExpense);
  let flowChartContent: React.ReactNode = (
    <div className="grid h-full place-items-center text-center text-sm text-muted-foreground">
      <div>
        <PieIcon className="mx-auto mb-3 size-9 opacity-30" />
  ...
  return (
    <div className="mx-auto max-w-[1360px] space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      <header className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={() => setReceiptOpen(true)}>
            <Receipt className="size-4" /> Emitir recibo
          </Button>
          <Button variant="outline" onClick={() => setNewType("despesa")}>
            <Plus className="size-4" /> Nova despesa
          </Button>
          <Button onClick={() => setNewType("receita")}>
      ...
      <div className="flex justify-end">
        <CapabilityNotice
          descriptor={{
            ...unavailable,
            key: "finance-layout",
            title: "Personalização do Financeiro",
            message:
              "A disposição atual segue o protótipo. A persistência de um layout financeiro personalizado ainda não está conectada.",
          }}
          trigger={
            <Button variant="outline" size="sm">
              <LayoutGrid className="size-4" /> Editar layout
            </Button>
          }
        />
      </div>
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
        <Card className="min-h-[560px] p-5">
          <Tabs
            value={tab}
            onValueChange={(value) =>
              patchQuery({ tab: value === "todos" ? null : value })
            }
          >
        ...
        <aside className="space-y-4">
          <Card className="p-4">
            <div className="mb-3 flex items-center justify-between border-b pb-3">
              <span className="flex items-center gap-1.5 text-xs uppercase tracking-wide text-muted-foreground">
                <Filter className="size-3.5" /> Resumo do período
              </span>
              <span className="text-[11px] text-muted-foreground">
                {period === "1m" ? "Mês atual" : "Período filtrado"}
              </span>
            </div>
            <Metric
              label="Saldo"
              value={money(effectiveRevenue - effectiveExpense)}
              icon={Wallet}
              tone="text-primary"
            />
            <Metric
              label="Receita efetivada"
              value={money(effectiveRevenue)}
              icon={TrendingUp}
              tone="text-success"
            />
            <Metric
              label="Despesa efetivada"
              value={money(effectiveExpense)}
              icon={TrendingDown}
              tone="text-warning"
            />
            <Metric
          ...
          <Card className="p-4">
            ...
            <div className="mt-2 flex w-fit rounded-md border p-0.5">
              <button
                type="button"
                className={cn(
                  "min-h-11 rounded px-2 py-1 text-xs",
                  chartKind === "donut" && "bg-primary text-primary-foreground",
                )}
                onClick={() => setChartKind("donut")}
              >
                <PieIcon className="mr-1 inline size-3" />
                Donut
              </button>
              <button
                type="button"
                className={cn(
                  "min-h-11 rounded px-2 py-1 text-xs",
                  chartKind === "bars" && "bg-primary text-primary-foreground",
                )}
            ...
            <div className="mt-3 h-64">{flowChartContent}</div>
          </Card>
        </aside>
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          <h2 className="font-semibold">Saldo acumulado</h2>
          <p className="text-xs text-muted-foreground">
          ...
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={daily}>
                ...
                <ChartTooltip formatter={(v) => money(Number(v) * 100)} />
                <Line
                  type="monotone"
                  dataKey="saldo"
                  stroke="var(--primary)"
        ...
        <Card className="p-5">
          ...
          <p className="text-xs text-muted-foreground">
            De onde sai o dinheiro do consultório
          </p>
      ...
      <FinanceEntryDialog
        open={newType !== null}
        onOpenChange={(open) => !open && setNewType(null)}
        initialType={newType ?? "receita"}
        patients={patients}
        defaultPatientId={search.get("patientId") ?? ""}
      />
      <FinanceEntryDialog
        open={editingEntry !== null}
        onOpenChange={(open) => !open && setEditingEntry(null)}
        initialType={editingEntry?.type ?? "receita"}
        patients={patients}
      ...
      <ReceiptDialog
        ...
        entries={entries.filter(
          (item) => item.type === "receita" && item.status === "efetivado",
        )}
      />
    </div>
  );
          ...
          {entries.length ? (
            entries.map((entry) => (
              <tr key={entry.id} className="border-b last:border-0">
                <td className="py-4">{dateBR(entry.date)}</td>
                <td className="max-w-72 truncate font-medium">
                  {entry.description}
                </td>
                <td>
                ...
                <td
                  className={
                    entry.type === "receita" ? "text-success" : "text-warning"
                  }
                >
                  {entry.type === "receita" ? "Receita" : "Despesa"}
                </td>
                <td>{payment(entry.paymentMethod)}</td>
                <td>
                  <Badge tone={financeEntryStatusTone(entry.status)}>
                    {financeEntryStatusLabel(entry.status)}
                  </Badge>
                </td>
                <td
                  className={cn(
                ...
                <td>
                  <div className="flex justify-end">
                    <FinanceEntryControls entry={entry} onEdit={() => onEdit(entry)} />
                  </div>
                </td>
              </tr>
            ))
...
function CategoriesPanel() {
  return (
    <div className="mt-5 grid gap-4 sm:grid-cols-2">
      <Card className="p-4">
        <h2 className="font-semibold">Categorias de receita</h2>
      ...
      <Card className="p-4">
        <h2 className="font-semibold">Categorias de despesa</h2>
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Selecione uma receita efetivada para montar o recibo.
            </DialogDescription>
          </DialogHeader>
          {entries.length ? (
              ...
              <SelectContent>
                {entries.map((entry) => (
                  <SelectItem key={entry.id} value={entry.id}>
                    {entry.patientName} · {dateBR(entry.date)} ·{" "}
                    {money(entry.valueCents)}
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            <div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">
              Nenhuma receita efetivada está disponível.
            </div>
          )}
          <DialogFooter>
            ...
            <Button disabled={!selected} onClick={() => setBlocked(true)}>
              Continuar
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <CapabilityNotice
        descriptor={{
          ...
          title: "Emissão de recibos em preparação",
          message:
            "O modelo visual está pronto, mas a geração do documento fiscal ainda não está conectada. Nenhum recibo foi emitido.",
        }}
        open={blocked}
