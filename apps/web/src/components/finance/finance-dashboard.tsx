...
export function FinanceDashboard({ entries, patients }: { entries: FinanceEntryView[]; patients: FinancePatientOption[] }) {
  ...
  const [newType, setNewType] = React.useState<"receita" | "despesa" | null>(() => search.get("new") === "despesa" ? "despesa" : search.get("new") === "receita" ? "receita" : null);
  ...
  return <div className="mx-auto max-w-[1360px] space-y-6 px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
      <Card className="min-h-[560px] p-5">
        ...
        </> : tab === "recibos" ? <EmptyPanel icon={Receipt} title="Nenhum recibo emitido" text="Selecione uma receita efetivada para emitir o primeiro recibo." action="Emitir recibo" onAction={() => setReceiptOpen(true)} /> : <CategoriesPanel />}
      </Card>
      <aside className="space-y-4"><Card className="p-4"><div className="mb-3 flex items-center justify-between border-b pb-3"><span className="flex items-center gap-1.5 text-xs uppercase tracking-wide text-muted-foreground"><Filter className="size-3.5" /> Resumo do período</span><span className="text-[11px] text-muted-foreground">{period === "1m" ? "Mês atual" : "Período filtrado"}</span></div><Metric label="Saldo" value={money(effectiveRevenue - effectiveExpense)} icon={Wallet} tone="text-primary" /><Metric label="Receita efetivada" value={money(effectiveRevenue)} icon={TrendingUp} tone="text-success" /><Metric label="Despesa efetivada" value={money(effectiveExpense)} icon={TrendingDown} tone="text-warning" /><Metric label="Receita prevista" value={money(expected)} icon={AlertCircle} tone="text-destructive" /></Card>
        <Card className="p-4"><div className="flex items-start justify-between"><div><h2 className="font-semibold">Fluxo financeiro</h2><p className="text-xs text-muted-foreground">Receitas vs. despesas — período</p></div></div><div className="mt-2 flex w-fit rounded-md border p-0.5"><button type="button" className={cn("min-h-11 rounded px-2 py-1 text-xs", chartKind === "donut" && "bg-primary text-primary-foreground")} onClick={() => setChartKind("donut")}><PieIcon className="mr-1 inline size-3" />Donut</button><button type="button" className={cn("min-h-11 rounded px-2 py-1 text-xs", chartKind === "bars" && "bg-primary text-primary-foreground")} onClick={() => setChartKind("bars")}><BarChart3 className="mr-1 inline size-3" />Barras</button></div><div className="mt-3 h-64">{effectiveRevenue || effectiveExpense ? <ResponsiveContainer width="100%" height="100%">{chartKind === "donut" ? <PieChart><Pie data={chartData} dataKey="value" nameKey="name" innerRadius={48} outerRadius={78}><Cell fill="var(--success)" /><Cell fill="var(--warning)" /></Pie><ChartTooltip formatter={(v) => money(Number(v))} /><Legend /></PieChart> : <BarChart data={chartData}><CartesianGrid vertical={false} strokeDasharray="3 3" /><XAxis dataKey="name" /><YAxis tickFormatter={(v) => `R$${Number(v) / 100}`} /><ChartTooltip formatter={(v) => money(Number(v))} /><Bar dataKey="value" fill="var(--primary)" radius={[5, 5, 0, 0]} /></BarChart>}</ResponsiveContainer> : <div className="grid h-full place-items-center text-center text-sm text-muted-foreground"><div><PieIcon className="mx-auto mb-3 size-9 opacity-30" />Nenhum lançamento efetivado no período.</div></div>}</div></Card>
      </aside>
    </div>
